...
mod tests {
    ...
    fn test_trr_single_frame() {
        ...
        let coord0 = frames[0].get_coord(patinae_mol::CoordIndex(0)).unwrap();
        assert!((coord0.x - 1.0).abs() < 0.01);
        assert!((coord0.y - 2.0).abs() < 0.01);
        assert!((coord0.z - 3.0).abs() < 0.01);

        let coord1 = frames[0].get_coord(patinae_mol::CoordIndex(1)).unwrap();
    ...
    fn test_trr_multi_frame() {
        ...
        let coord = frames[1].get_coord(patinae_mol::CoordIndex(0)).unwrap();
    ...
    fn test_trr_frame_filtering() {
        ...
        let c0 = frames[0].get_coord(patinae_mol::CoordIndex(0)).unwrap();
        assert!((c0.x - 1.0).abs() < 0.01);

        // Frame at index 3: x = 0.3 nm = 3.0 Å
        let c1 = frames[1].get_coord(patinae_mol::CoordIndex(0)).unwrap();
