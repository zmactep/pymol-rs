...
impl ViewManager {
    ...
    /// # Errors
    ///
    /// Returns an error if no view exists for `key`.
    pub fn recall(&mut self, key: &str, camera: &mut Camera, animate: f32) -> SceneResult<()> {
        let view = self
            .views
            .get(key)
            .ok_or_else(|| SceneError::view_not_found(key))?
            .clone();
    ...
    /// # Errors
    ///
    /// Returns an error if no views are stored.
    pub fn recall_next(&mut self, camera: &mut Camera, animate: f32) -> SceneResult<()> {
        let key = self
            .next()
            .ok_or_else(|| SceneError::view_not_found("no views"))?
            .to_string();
    ...
    /// # Errors
    ///
    /// Returns an error if `old_key` does not exist or `new_key` already exists.
    pub fn rename(&mut self, old_key: &str, new_key: &str) -> SceneResult<()> {
        if !self.views.contains_key(old_key) {
            return Err(SceneError::view_not_found(old_key));
        }

        if self.views.contains_key(new_key) {
            return Err(SceneError::view_exists(new_key));
        }
