...
pub fn ce_align(
    ...
    let (pairs, rmsd) =
        select_best_path(source_ca, target_ca, &paths, w).ok_or(AlignError::NoMatches)?;
    ...
    for i_a in 0..=len_a.saturating_sub(w) {
        for i_b in 0..=len_b.saturating_sub(w) {
            let score: f32 = (0..(w - 2))
                ...
                .map(|(row, col)| (dm_a[i_a + row][i_a + col] - dm_b[i_b + row][i_b + col]).abs())
                .sum();
...
fn resolve_gap(last: (usize, usize), win_size: usize, gap_idx: usize) -> (usize, usize) {
    let offset = gap_idx.div_ceil(2);
    let (mut j_a, mut j_b) = (last.0 + win_size, last.1 + win_size);
    if (gap_idx + 1).is_multiple_of(2) {
        j_a += offset;
    } else {
        j_b += offset;
    }
...
fn fragment_d_score(
    ...
    let total: f64 = path
        ...
        .map(|&(pa, pb)| {
            ...
            let last =
                (dm_a[pa + w - 1][j_a + w - 1] as f64 - dm_b[pb + w - 1][j_b + w - 1] as f64).abs();
            let mid: f64 = (1..(w - 1))
                .map(|k| {
                    (dm_a[pa + k][j_a + w - 1 - k] as f64 - dm_b[pb + k][j_b + w - 1 - k] as f64)
                        .abs()
                })
                .sum();
            first + last + mid
...
fn grow_path(
    inputs: &PathGrowthInputs<'_>,
    seed: (usize, usize),
    seed_score: f64,
) -> Vec<(Vec<(usize, usize)>, f64)> {
    let w = inputs.params.win_size;
    let d0 = inputs.params.d0 as f64;
    let d1 = inputs.params.d1 as f64;
    let win_sum = ((w - 1) * (w - 2)) / 2;
    let n_gaps = inputs.params.gap_max * 2 + 1;
    ...
    loop {
        ...
        let best = (0..n_gaps)
            .filter_map(|g| {
                ...
                if j_a + w > inputs.len_a || j_b + w > inputs.len_b {
                    return None;
                }
                let afp_sim = inputs.sim[j_a][j_b] as f64;
                if afp_sim >= d0 || afp_sim < 0.0 {
                    return None; // Eq. 9: D_an < D_0
                }
                let d = fragment_d_score(&path, j_a, j_b, inputs.dm_a, inputs.dm_b, w);
        ...
        let step = path.len();
        let cumulative = (prev * inputs.norm[step - 1]
            + score1 * (inputs.norm[step] - inputs.norm[step - 1]))
            / inputs.norm[step];
...
fn extend_paths(
    ...
    let norm: Vec<f64> = (0..smaller)
        ...
        .collect();
    let growth_inputs = PathGrowthInputs {
        sim,
        dm_a,
        dm_b,
        len_a,
    ...
    for (i_a, sim_row) in sim.iter().enumerate().take(len_a) {
        // Early termination: can't form a longer path than the best found
        if best_len > 1 && i_a + w * (best_len - 1) > len_a {
            break;
        }

        for (i_b, &seed_score) in sim_row.iter().enumerate().take(len_b) {
            let seed_score = seed_score as f64;
            ...
            let snapshots = grow_path(&growth_inputs, (i_a, i_b), seed_score);
