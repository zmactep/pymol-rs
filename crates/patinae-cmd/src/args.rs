...
pub struct ParsedCommand {
    ...
    /// Raw argument text after the command name, when parsed from user input.
...
impl ParsedCommand {
    ...
    /// Set the raw argument text captured from user input.
    pub fn with_raw_args(mut self, raw_args: impl Into<String>) -> Self {
        self.raw_args = Some(raw_args.into());
        self
    }

    /// Get the raw argument text captured from user input.
