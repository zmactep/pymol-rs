...
impl Pattern {
    ...
    pub fn matches(&self, value: &str, case_sensitive: bool) -> bool {
        match self {
            ...
            Pattern::Wildcard(pattern) => Self::match_wildcard(pattern, value, case_sensitive),
            Pattern::List(patterns) => patterns.iter().any(|p| p.matches(value, case_sensitive)),
    ...
    pub fn matches(&self, resv: i32, inscode: char) -> bool {
        match self {
            ...
            ResiItem::InsCodeRange(start_v, start_c, end_v, end_c) => {
                // This simplified implementation compares residue numbers
                // first and insertion codes only for exact endpoints.
